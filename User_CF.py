...
def get_data():
    # 读取数据
    ratings = pd.read_csv('mov-rating.csv')

    # 分割训练和验证集
    trn_data, val_data, _, _ = train_test_split(ratings, ratings, test_size=0.2)
    trn_data = trn_data.groupby('userID')['movieID'].apply(list).reset_index()
    val_data = val_data.groupby('userID')['movieID'].apply(list).reset_index()

    trn_user_items = {}
    val_user_items = {}

    # 将数组构造成字典的形式{user_id: [item_id1, item_id2,...,item_idn]}
    for user, movies in zip(*(list(trn_data['userID']), list(trn_data['movieID']))):
        trn_user_items[user] = set(movies)

    for user, movies in zip(*(list(val_data['userID']), list(val_data['movieID']))):
